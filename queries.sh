...
echo "$($PSQL "select name from games left join teams on games.winner_id = teams.team_id where year=2014 and round='Eighth-Final' group by name union select name from games left join teams on games.opponent_id = teams.team_id where year=2014 and round='Eighth-Final' group by name order by name;")";
